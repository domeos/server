(function (domeApp, undefined) {
    ...
    domeApp.controller('MonitorCtr', ['$scope', '$domeCluster', '$domeDeploy', '$domeMonitor', '$domePublic', '$modal', '$domeUser', '$q', '$sce', function ($scope, $domeCluster, $domeDeploy, $domeMonitor, $domePublic, $modal, $domeUser, $q, $sce) {
        'use strict';
                    ...
                    if (alarmUsers && alarmUsers.length !== 0) {
                        for (var i = 0, l = alarmUsers.length; i < l; i++) {
                            if (alarmUsers[i].userId == user.id) {
                                $scope.alarmPermission = {
                                    id: user.id,
                                    name: alarmUsers[i].username,
                                    role: alarmUsers[i].role
                                };
                                break;
                            }
                        }
        ...
        var freshNodeMonitor = function () {
            ...
            nodeService.getNodeList($scope.clusterListIns.cluster.id).then(function (res) {
        ...
        $scope.toggleDeploy = function (deployId, deployName, namespace) {
            $scope.deployListIns.toggleDeploy(deployId, deployName, namespace).finally(function () {
                freshPodMonitor();
            });
    ...
    }]).controller('ContainersModalCtr', ['$scope', 'instanceIns', 'monitorParams', '$domeMonitor', '$domePublic', function ($scope, instanceIns, monitorParams, $domeMonitor, $domePublic) {
