		...
		deleteUser: function(user) {
			...
			function spliceUser() {
				for (var i = 0; i < that.resourceInfo.userInfos.length; i++) {
					if (that.resourceInfo.userInfos[i].userId === user.userId) {
			...
			if (that.resourceInfo.resourceType == 'group') {
				deleteGroupUser(that.resourceInfo.resourceId, user.userId).then(function(res) {
					spliceUser();
				}, function(res) {
				...
				$domePublic.openDelete().then(function() {
					deleteResourceUser(that.resourceInfo.resourceType, that.resourceInfo.resourceId, user.ownerType, user.userId).then(function(res) {
						spliceUser();
					}, function(res) {
						$domePublic.openWarning({
							title: '删除失败！',
