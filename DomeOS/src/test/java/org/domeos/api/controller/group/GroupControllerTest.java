...
public class GroupControllerTest extends BaseTestCase {
    ...
    private String getRoleJsonStr(RoleType roleType) {
        String jsonStr = String.format("{\"role\":\"%s\"}", roleType.getRoleName());
        return jsonStr;
    }
    ...
    public void T000CreateGroup() throws Exception {
        login("csf", "csf");
        String jsonStr = readInJsonFromFile("./src/test/resources/group/create_group.json");
        ...
        jsonStr = readInJsonFromFile("./src/test/resources/group/create_group2.json");
        group = objectMapper.readValue(jsonStr.getBytes(), Group.class);
        mockMvc.perform(MockMvcRequestBuilders
                .post("/api/group/create").contentType(MediaType.APPLICATION_JSON).content(jsonStr))
                .andDo(print())
    ...
    public void T003AddUserToGroup() throws Exception {
        login("csf", "csf");
        String jsonStr = readInJsonFromFile("./src/test/resources/group/user_group.json");
...
//        User user = (User) res.getResult();
        User user = userService.getUser("csf");
        String jsonStr = readInJsonFromFile("./src/test/resources/group/create_group2.json");
