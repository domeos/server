...
import org.domeos.util.code.GitWebHook;
import org.domeos.util.code.WebHook;
...
public class BuildControllerTest extends BaseTestCase {
    ...
    public void setup() throws IOException {
        ...
        FileInputStream buildInfoStream = new FileInputStream("./src/test/resources/ci/buildInfo.json");
        byte[] buffInfo = new byte[buildInfoStream.available()];
        buildInfoStream.read(buffInfo);
        buildInfo = objectMapper.readValue(buffInfo, BuildInfo.class);
        buildInfoStr = new String(buffInfo);

        FileInputStream buildStatusStream = new FileInputStream("./src/test/resources/ci/buildStatus.json");
        byte[] buffStatus = new byte[buildStatusStream.available()];
        buildStatusStream.read(buffStatus);
        buildStatus = objectMapper.readValue(buffStatus, BuildStatus.class);
        buildStatusStr = new String(buffStatus);

        FileInputStream webhookStream = new FileInputStream("./src/test/resources/ci/webhook.json");
    ...
    public void T030SetBuildStatus() throws Exception {
        mockMvc.perform(
                post("/api/ci/build/status").contentType(MediaType.APPLICATION_JSON).content(buildStatusStr))
                .andDo(print())
                .andExpect(status().isOk());
    }
