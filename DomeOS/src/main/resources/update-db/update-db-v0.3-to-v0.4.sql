...
domeos:BEGIN
  ...
  IF x=0 THEN
    INSERT INTO project_collection (name, description, state, createTime, removeTime, removed, data, projectCollectionState)
    ...
    INSERT INTO operation_history (resourceId, resourceType, operation, userId, userName, status, message, operateTime)
      VALUES (collection_id, "PROJECT_COLLECTION", "SET", creator_id, creator_name, "OK", "", cur_time);
  END IF;
  SELECT id INTO collection_id FROM project_collection WHERE name = collection_name;
  INSERT INTO collection_resource_map (resourceId, creatorId, resourceType, collectionId, updateTime)
...
domeos:BEGIN
  ...
  IF x=0 THEN
    ...
    INSERT INTO operation_history (resourceId, resourceType, operation, userId, userName, status, message, operateTime) VALUES (collection_id, "PROJECT_COLLECTION", "SET", creator_id, creator_name, "OK", "", cur_time);
    IF isnull(group_id) THEN
      INSERT IGNORE INTO collection_authority_map (collectionId, resourceType, userId, role, updateTime) VALUES (collection_id, "PROJECT_COLLECTION", creator_id, "MASTER", cur_time);
    ELSE
    INSERT INTO collection_authority_map (collectionId, resourceType, userId, role, updateTime) SELECT collection_id, "PROJECT_COLLECTION", user_group_map.userId, user_group_map.role, cur_time FROM user_group_map WHERE groupId = group_id;
    END IF;
   END IF;
  INSERT INTO collection_resource_map (resourceId, creatorId, resourceType, collectionId, updateTime) VALUES (resource_id, creator_id, "PROJECT", collection_id, cur_time);
...
CREATE PROCEDURE update_deploy_user()
    ...
    process: LOOP
      ...
      INSERT INTO deploy_collection (name, description, state, createTime, removeTime, removed, data)
      ...
      INSERT INTO collection_authority_map (collectionId, resourceType, userId, role, updateTime)
...
CREATE PROCEDURE update_deploy_group()
    ...
    process: LOOP
      ...
      SELECT id INTO collection_id FROM deploy_collection WHERE name = collection_name;
      INSERT INTO collection_resource_map (resourceId, creatorId, resourceType, collectionId, updateTime) SELECT resources.resourceId, operation_history.userId, "DEPLOY", collection_id, cur_time
      ...
      INSERT INTO operation_history (resourceId, resourceType, operation, userId, userName, status, message, operateTime) VALUES (collection_id, "DEPLOY_COLLECTION", "SET", 1, "admin", "OK", "", cur_time);
    END LOOP process;
...
INSERT INTO collection_authority_map (collectionId, resourceType, userId, role, updateTime) SELECT
                                                                                              1000, "ALARM", userId, role, updateTime FROM user_group_map WHERE groupId = 1000;

INSERT INTO alarm_user_group_info (userGroupName, creatorId, creatorName, createTime, updateTime) SELECT
...
INSERT INTO alarm_user_group_user_bind (userGroupId, userId, bindTime)
...
INSERT INTO collection_authority_map ( collectionId, resourceType, userId, role, updateTime ) SELECT res.resourceId, res.resourceType, res.ownerId, res.role, res.updateTime FROM resources AS res WHERE res.resourceType = 'CLUSTER' AND res.ownerType='USER';

INSERT INTO collection_authority_map ( collectionId, resourceType, userId, role, updateTime ) SELECT res.resourceId, res.resourceType, ugm.userId, ugm.role, res.updateTime FROM resources AS res INNER JOIN user_group_map AS ugm ON res.resourceType = 'CLUSTER' AND res.ownerType='GROUP' AND res.ownerId=ugm.groupId;

#insert cluster data(cluster) into collection_resource_map
INSERT INTO collection_resource_map ( resourceId, creatorId, resourceType, collectionId, updateTime ) SELECT clus.id,
