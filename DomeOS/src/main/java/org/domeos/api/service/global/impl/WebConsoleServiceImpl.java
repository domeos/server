...
public class WebConsoleServiceImpl implements WebConsoleService {
    ...
    @Override
    public HttpResponseTemp<?> getWebsshSetting(long userId) {
        if (!AuthUtil.isAdmin(userId)) {
            return ResultStat.FORBIDDEN.wrap(null);
        }
        WebSsh webSsh = globalService.getWebSsh();
        return ResultStat.OK.wrap(webSsh);
    }

    @Override
    public HttpResponseTemp<?> setWebsshSetting(WebSsh webSsh, long userId) {
        if (!AuthUtil.isAdmin(userId)) {
            return ResultStat.FORBIDDEN.wrap(null);
        }
        ...
        globalService.deleteWebSsh();
        globalService.setWebSsh(webSsh);
        return ResultStat.OK.wrap(webSsh);
    }

    @Override
    public HttpResponseTemp<?> updateWebsshSetting(WebSsh webSsh, long userId) {
        if (!AuthUtil.isAdmin(userId)) {
            return ResultStat.FORBIDDEN.wrap(null);
        }
        if (!StringUtils.isBlank(webSsh.checkLegality())) {
        ...
        globalService.updateWebSsh(webSsh);
        return ResultStat.OK.wrap(webSsh);
    }

    @Override
    public HttpResponseTemp<?> deleteWebsshSetting(long userId) {
        if (!AuthUtil.isAdmin(userId)) {
            return ResultStat.FORBIDDEN.wrap(null);
        }
        globalService.deleteWebSsh();
