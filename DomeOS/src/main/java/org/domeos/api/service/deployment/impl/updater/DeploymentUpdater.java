...
import org.domeos.api.service.deployment.impl.DeploymentServiceImpl;
...
public class DeploymentUpdater {
    ...
    public DeploymentUpdater(KubeClient client, Deployment deployment, Version version) {
        this.client = client;
        this.deployment = deployment;
        this.dstVersion = version;
        this.isKeepQuantityIdentify = true;
        this.targetRC = DeploymentServiceImpl.buildReplicationController(deployment, version);
    }

    public DeploymentUpdater(KubeClient client, Deployment deployment, Version version, int replicas) {
        this.client = client;
        this.deployment = deployment;
        this.dstVersion = version;
        ...
        synchronized (status) {
            if (status.getPhase() != DeploymentUpdatePhase.Unknown) {
                String message = "try start one updater which has been start, deployId="
                        + deployment.getDeployId() + ", dstVersionId=" + dstVersion.getVersion();
    ...
    public void updateDeployment() {
        try {
            ...
            if (targetRCList == null || targetRCList.getItems() == null || targetRCList.getItems().length == 0) {
                ...
                failedPhase("update deployment(id=" + deployment.getDeployId()
