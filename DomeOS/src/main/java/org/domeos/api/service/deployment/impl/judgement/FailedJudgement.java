package org.domeos.api.service.deployment.impl.judgement;
...
public class FailedJudgement {
    ...
    public boolean isFailed(Pod pod) throws ParseException {
        ...
        if (pod.getStatus().getContainerStatuses() == null) {
            ...
            startTime = DateManager.string2timestamp(pod.getMetadata().getCreationTimestamp(),
                    TimeZone.getTimeZone(GlobalConstant.UTC_TIME));
        } else {
            startTime = DateManager.string2timestamp(pod.getStatus().getStartTime(), TimeZone.getTimeZone(GlobalConstant.UTC_TIME));
        }
