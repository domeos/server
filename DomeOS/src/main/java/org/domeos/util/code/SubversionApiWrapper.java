...
public class SubversionApiWrapper implements CodeApiInterface {
    ...
    public void initWithsvnId(int svnId){
        this.svnId = svnId;
        Subversion subversion = SubversionInfo.getSubversion(svnId);
        if(subversion != null) {
   ...
   public List<CodeSourceInfo> listCodeInfo(int userId){
        ...
        List<Subversion> subversions = SubversionInfo.getSubversionsByUserId(userId);
        if (subversions != null) {
    ...
    public GitlabApiWrapper.CommitInfo getCommitInfo(int svnId, String path){
        initWithsvnId(svnId);
        init();
        GitlabApiWrapper.CommitInfo info = new GitlabApiWrapper.CommitInfo();
        try {
    ...
    public GitlabApiWrapper.CommitInfo getTagCommitInfo(int projectId, String tag){
        return getCommitInfo(projectId, "tags/"+tag);

    }

    public GitlabApiWrapper.CommitInfo getBranchCommitInfo(int projectId, String branch){
