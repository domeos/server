package org.domeos.framework.api.mapper.collection;
...
public interface CollectionAuthorityMapMapper {
    ...
    @Select("SELECT * from " + CollectionBiz.COLLECTION_AUTHORITY_MAP_NAME +
            " WHERE name=#{name} AND resourceType = #{resourceType}")
    CollectionAuthorityMap getCollectionByNameAndResourceType(@Param("name") String name,
                                                              @Param("resourceType") ResourceType resourceType);

    @Select("SELECT c.id, c.name, c.description, c.createTime from ${tableName} c, " + CollectionBiz.COLLECTION_AUTHORITY_MAP_NAME +
            " ca WHERE ca.userId=#{userId} AND ca.collectionId=c.id AND c.removed=0" )
