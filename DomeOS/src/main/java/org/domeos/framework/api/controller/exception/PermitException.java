...
public class PermitException extends RuntimeException {
    ...
    public PermitException(int userId, int collectionId, OperationType operationType, int dstUserId) {
        super(String.format("permit fatal, user:%d operate:%s on collection_id:%d dst_user_id:%s", userId, operationType, collectionId, dstUserId));
    }
}
