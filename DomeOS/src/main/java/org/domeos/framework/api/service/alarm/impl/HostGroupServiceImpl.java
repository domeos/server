...
public class HostGroupServiceImpl implements HostGroupService {
    ...
    public HttpResponseTemp<?> listHostGroupInfo() {
        ...
        List<HostGroupInfo> hostGroupInfos = new LinkedList<>();
        List<Future<HostGroupInfo>> futures = new LinkedList<>();

        for (HostGroupInfoBasic hostGroupInfoBasic : hostGroupInfoBasics) {
            Future<HostGroupInfo> future = ClientConfigure.executorService.submit(new HostGroupInfoTask(hostGroupInfoBasic));
            futures.add(future);
        }
        for (Future<HostGroupInfo> future : futures) {
            try {
                HostGroupInfo info = future.get();
                if (info != null) {
                    hostGroupInfos.add(info);
                }
            } catch (InterruptedException | ExecutionException e) {
                logger.warn("get host group list error, message is " + e.getMessage());
            }
        }
