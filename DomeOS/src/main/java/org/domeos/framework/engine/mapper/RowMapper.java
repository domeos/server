...
public interface RowMapper extends ISimpleMapper{
    ...
    String basicColumns = " id, name,  description, state, createTime, removeTime, removed , data ";


    @Select("select" + basicColumns + " from ${tableName} where id=${id}")
    RowMapperDao getById(@Param("tableName") String tableName, @Param("id") int id);

    @Select("select" + basicColumns + " from ${tableName} where name='${name}' and removed=0 limit 1")
    RowMapperDao getByName(@Param("tableName") String tableName, @Param("name") String name);

    @Select("select" + basicColumns + " from ${tableName} where removed = 0")
    List<RowMapperDao> getList(@Param("tableName") String tableName);

    @Select("select" + basicColumns + " from ${tableName} where name='${name}' and removed = 0")
    List<RowMapperDao> getListByName(@Param("tableName") String tableName, @Param("name") String name);

    @Select("select" + basicColumns + " from ${tableName} where id in ${idList} and removed = 0")
