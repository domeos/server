...
(function (domeApp, undefined) {
    ...
    $scope.deleteVolumeDraft = function (volume, images) {
      $scope.deployIns.deleteVolumeDraft(volume, images);
      if ($scope.currentVolumeDraft.index > $scope.deployIns.config.volumeDrafts.length - 1) {
        $scope.currentVolumeDraft.index = 0;
      }
    };
    $scope.addVolumeDraft = function () {
        $scope.deployIns.addVolumeDraft();
        $scope.currentVolumeDraft.index = $scope.deployIns.config.volumeDrafts.length - 1;
    };
    $scope.currentVolumeDraft = {
      index: 0
    };
