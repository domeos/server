...
public class GitlabApiWrapper implements CodeApiInterface {
    ...
    private class CodeSourceTask implements Runnable {
        ...
        public CodeSourceTask(Set<CodeSourceInfo> set, int id, String userName, String token, int gitlabId) {
            this.set = set;
            this.id = id;
    ...
    public List<CodeSourceInfo.ProjectInfo> getGitlabProjectInfos() {
        ...
        try {
            ...
            if (projects != null) {
                List<ProjectInfoTask> projectInfoTasks = new ArrayList<>();
                for (GitlabProject project : projects) {
                    projectInfoTasks.add(new ProjectInfoTask(projectInfos, project, 30));
                }
                ClientConfigure.executeRunnableCompletionService(projectInfoTasks);
            }
        } catch (IOException ignored) {
