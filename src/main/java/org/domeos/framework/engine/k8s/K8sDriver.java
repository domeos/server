...
public class K8sDriver implements RuntimeDriver {
    ...
    public void startDeploy(Deployment deployment, Version version, User user, List<LoadBalancer> lbs, List<EnvDraft> allExtraEnvs)
        ...
        try {
            for (LoadBalancer loadBalancer : lbs) {
                Service service = new K8sServiceBuilder(loadBalancer).build();
                if (service == null) {
            ...
            deployResourceHandler.create(version, lbs, allExtraEnvs);
        } catch (Exception e) {
    ...
    public void rollbackDeploy(Deployment deployment, int versionId, List<EnvDraft> allExtraEnvs, User user, Policy policy)
        ...
        Boolean status = checkLoadBalanceStatus(kubeUtils, lbs);
        if (!status) {
            deploymentStatusManager.failedEvent(eventId, currentRunningSnapshot,
                    "Bad loadBalancer service info with deployment=" + deployment.getName());
        }
        // create secret before the create of rc
        // judge the registry is belong to domeos or not
    ...
    private boolean checkLoadBalanceStatus(KubeUtils kubeUtils, List<LoadBalancer> lbs) throws DeploymentEventException {
        if (lbs != null && lbs.size() > 0) {
            for (LoadBalancer loadBalancer : lbs) {
                ...
                Service oldService;
                try {
                    oldService = kubeUtils.serviceInfo(service.getMetadata().getName());
                } catch (K8sDriverException e) {
                    throw new DeploymentEventException("kubernetes exception with message=" + e.getMessage());
                }
                if (oldService == null) {
