...
public class DaemonSetDeployHandler implements DeployResourceHandler<DaemonSet> {
    ...
    @Override
    public DaemonSet create(Version version, List<LoadBalancer> loadBalancers, List<EnvDraft> extraEnvs) throws K8sDriverException {
        DaemonSet daemonSet = build(version, loadBalancers, extraEnvs);
    ...
    public void deleteAllDaemonSet() throws K8sDriverException {
        ...
        if (dsList != null && dsList.getItems() != null) {
            for (DaemonSet ds : dsList.getItems()) {
                kubeUtils.deleteDaemonSet(ds.getMetadata().getName());
            }
        }
