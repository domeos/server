...
public interface PortalGroupMapper {
    @Insert("INSERT INTO portal.grp (grp_name, create_user, create_at, come_from) VALUES (" +
            "#{grp_name}, #{create_user}, #{create_at}, #{come_from})")
    @Options(useGeneratedKeys = true, keyProperty = "id", keyColumn = "id")
    int insertHostGroup(Group group);

    @Insert("INSERT INTO portal.grp (id, grp_name, create_user, create_at, come_from) VALUES (" +
            "#{id}, #{grp_name}, #{create_user}, #{create_at}, #{come_from})")
    int insertHostGroupById(Group group);

    @Update("UPDATE portal.grp SET grp_name=#{grp_name} WHERE id=#{id}")
    int updateHostGroup(@Param("id") long id, @Param("grp_name") String grp_name);

    @Delete("DELETE FROM portal.grp WHERE id=#{id}")
    int deleteHostGroup(@Param("id") long id);

    @Delete("DELETE FROM portal.grp WHERE id IN (SELECT grp_id FROM portal.grp_tpl WHERE tpl_id=#{tpl_id})")
    int deleteByTemplate(@Param("tpl_id") long tpl_id);

    @Select("SELECT * FROM portal.grp")
