...
public interface BuildHistoryMapper {
    ...
    @Select("SELECT * FROM build_history WHERE projectId=#{projectId} ORDER BY id DESC LIMIT #{start}, #{count}")
    List<RowMapperDao> getPageByProjectId(@Param("projectId") int projectId, @Param("start") int start, @Param("count") int count);

    @Select("SELECT * FROM " + GlobalConstant.BUILDHISTORY_TABLE_NAME + " bh "
