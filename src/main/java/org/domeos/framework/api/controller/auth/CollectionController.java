...
public class CollectionController extends ApiController {
    ...
    @ResponseBody
    @RequestMapping(value = "/collectionspace/list/{resourceType}", method = RequestMethod.GET)
    public HttpResponseTemp<?> getCollectionSpace(@PathVariable ResourceType resourceType) {
        int userId = AuthUtil.getUserId();
        return userCollectionService.getCollectionSpace(userId, resourceType);
    }
