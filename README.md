...
样例: 
  ...
  则需要在10.11.150.1主机上执行sudo sh start_etcd.sh 1
  
  在10.11.150.2主机上执行sudo sh start_etcd.sh 2
  
  在10.11.150.3主机上执行sudo sh start_etcd.sh 3
  
测试:

  通过" curl -L <结点IP>:<ETCD服务端口>/health "查看各个节点服务状态，如果返回的health为true说明该节点上的ETCD正常，如:
  
    command: curl -L 10.11.150.1:4012/health
    result : {"health": "true"}
...
样例:
    ...
    sudo docker run --restart=always -d -p 5000:5000 -e REGISTRY_STORAGE_SOHUSTORAGE_ACCESSKEY=f5ynwwOAAA2yT4+qxzmA6A== -e REGISTRY_STORAGE_SOHUSTORAGE_SECRETKEY=MUC9NeF8vXvBBB2f+6dIXA== -e REGISTRY_STORAGE_SOHUSTORAGE_REGION=bjcnc -e REGISTRY_STORAGE_SOHUSTORAGE_BUCKET=registry --name private-registry pub.domeos.org/domeos/docker-registry-driver-sohustorage:1.0

测试:

  通过" curl -L http://<registry的服务地址>/v2/ "查看docker registry的运行状态，如果显示"{}"说明正常运行，如:

    command: curl -L http://10.11.150.1:5000/v2/
    result : {}
...
样例:
    ...
    graph: sudo docker run -d -v /opt/my/data:/home/work/data/6070 -e DB_DSN="\"domeos:xxxx@tcp(10.11.150.1:3306)/graph?loc=Local&parseTime=true\"" --name graph -p 6070:6070 -p 6071:6071 --restart=always pub.domeos.org/domeos/graph:0.5.7

    transfer: sudo docker run -d -e JUDGE_ENABLE="false" -e GRAPH_CLUSTER="\"node-00\":\"10.11.150.1:6070\",\"node-01\":\"10.11.150.2:6070\"" -p 8433:8433 --name transfer --restart=always pub.domeos.org/domeos/transfer:0.0.15

    query: sudo docker run -d -e GRAPH_CLUSTER="\"node-00\":\"10.11.150.1:6070\",\"node-01\":\"10.11.150.2:6070\"" -p 9966:9966 --name query --restart=always pub.domeos.org/domeos/query:1.5.1
...
测试:
  ...
  通过" curl -s <graph/transfer/query的服务地址>/health "查看graph/transfer/query的运行状态，如果返回"ok"说明graph/transfer/query正常运行，注意graph中使用的是_graph_http_port端口，如:
    ...
    command: curl -s 10.11.150.1:6071/health
    result : ok
...
容器方式:
  ...
  命令: 
    ...
    sudo docker run -d --restart=always --name <_domeos_server_name> -p <_domeos_server_port>:8080 -e MYSQL_HOST=<_mysql_host> -e MYSQL_PORT=<_mysql_port> -e MYSQL_USERNAME=<_mysql_username> -e MYSQL_PASSWORD=<_mysql_password> -e MYSQL_DB=<_mysql_db> pub.domeos.org/domeos/server:1.2.3
  ...
  样例:
    ...
    sudo docker run -d --restart=always --name domeos_server -e MYSQL_HOST=10.11.150.1 -e MYSQL_PORT=3306 -e MYSQL_USERNAME=domeos -e MYSQL_PASSWORD=mypassword -e MYSQL_DB=domeos -p 8080:8080 pub.domeos.org/domeos/server:1.2.3
...
测试:
  ...
  通过"curl -s <WebSSH服务地址>"查看WebSSH服务状态，如果返回了一个"File Not Found"的页面信息而不是返回空，说明WebSSH服务运行正常，如:
    ...
    command: curl -s 10.11.150.1:4200
    result :
...
## STEP 08: DomeOS系统录入参数
  ...
  全局配置->"LDAP"->OpenLDAP服务器地址
    OpenLDAP的服务地址以及端口号，样例: ldap://ldap.test.com  389
	
  全局配置->"LDAP"->email后缀
    OpenLDAP登录时的E-mail后缀，样例: @test.com
	
  全局配置->"代码仓库"->代码仓库地址
    GitLab的地址，样例: http://test.com
	
  全局配置->"私有仓库"->私有仓库地址
    Docker registry的地址，样例: http://10.11.150.1:5000 或 pub.domeos.org
    如果使用https访问的私有仓库，则勾选"https"并将registry.crt证书文件内容粘贴至"证书信息"中
	
  全局配置->"服务器"->服务器地址
    DomeOS Server的地址，样例: http://10.11.150.1:8080
	
  全局配置->"监控配置"-> transfer
    transfer服务地址，样例 10.11.150.1:8433
	
  全局配置->"监控配置"-> graph
    graph服务地址，样例 10.11.150.1:6070
	
  全局配置->"监控配置"-> query
    query服务地址，样例 10.11.150.1:9966
	
  全局配置->"Web SSH"->Web SSH服务地址
    如果DomeOS Server以非容器方式启动，则这项设置为单独启动的WebSSH服务地址，样例:http://10.11.150.1:4200
    如果DomeOS Server以容器方式启动，则WebSSH服务地址必须设置为: http://localhost:4200

  集群管理->新建集群->api server
    kube-apiserver服务地址，样例:10.11.150.1:8080
	
  集群管理->新建集群->dns服务器
    kubernetes集群内DNS服务地址，注意不能添加端口号，样例:172.16.40.1
	
  集群管理->新建集群->etcd
    ETCD集群服务地址，注意必须加http://前缀，样例:http://10.11.150.1:8080
...
样例:
    ...
    sudo sh start_node_centos.sh --api-server http://10.11.150.1:8080 --cluster-dns 172.16.40.1 --cluster-domain domeos.local --monitor-transfer 10.11.150.1:8433,10.11.150.2:8433 --docker-graph-path /opt/domeos/openxxs/docker-graph --docker-log-level warn --registry-type http --registry-arg 10.11.150.1:5000 --domeos-server 10.11.150.1:8080 --etcd-server http://10.11.150.1:4012 --node-labels TESTENV=HOSTENVTYPE,PRODENV=HOSTENVTYPE
  
测试:

  通过" kubectl --server <kube-apiserver的服务地址> get node "查看集群的node节点状态，如果显示了该结点并且状态为"Ready"，则说明node节点添加成功，如:

    command: kubectl --server 10.11.150.1:8080 get nodes
    result :
    NAME         STATUS    AGE
...
样例:
    ...
    kubectl --server 10.11.150.1:8080 create -f dns.yaml
	
测试:

  1. 通过" kubectl --server 10.11.150.1:8080 get svc "获取集群svc列表，确认skydns-svc是否已创建
  
  2. 通过" kubectl --server 10.11.150.1:8080 get pods "获取集群pod列表，确认skydns-为前缀和kube2sky-为前缀的两个pod是否处于Running状态，如skydns-u44ey和kube2sky-2h1b9
